...
Replace implicit reliance on `narduk-core`'s Nuxt auto-imports
...
default options (`app: true`). A consumer that narrows the module surface
(for example `{ app: false, server: true }`, used by `spacex-ipo` to avoid a
