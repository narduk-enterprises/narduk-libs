...
(fixes an `UNLOADABLE_DEPENDENCY` build failure), a pinned
...
scripts plus the `@narduk-enterprises/narduk-app-tools` devDependency that back
them, a generated `apps/web/scripts/validate-manifests.mjs` pre-deploy check,
new `CONTRACT.md` and `docs/workers-builds.md` templates, a Playwright
