...
narduk-seo no longer hard-depends on nuxt-og-image. The package is an optional peer at 6.8.0. Static-card apps omit it and set `ogImage.enabled: false` or `ogImage.zeroRuntime: true`; runtime OG apps add `nuxt-og-image@6.8.0` themselves. If the peer is missing, the layer skips `installModule`, registers a no-op `defineOgImage`, and `useSeo` falls back to the static image.
