...
Moved from the standalone `narduk-mapkit` repository (`packages/nuxt`) into
`narduk-libs` at `packages/modules/narduk-mapkit-nuxt`, with full git history
preserved. The package name, version line, module entry points and runtime
behaviour are unchanged. It now runs the shared
