...
- `defineStatusMap<T extends string>(map)` turns a domain-specific status union
  ...
  `{ tone: 'neutral', label: '<raw key>' }` instead of throwing. It is a named
  export of the package root (`src/module.ts`) and a Nuxt auto-import via
  `addImports`. No fourth subpath.
