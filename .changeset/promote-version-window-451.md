...
the new `--max-versions` flag, default 500 -- and never paginates unbounded. It
needs an account id and `CLOUDFLARE_API_TOKEN`; without both it falls back to
`wrangler versions list` and reports `versionSearch.source: "wrangler"` so a
miss in ten is never mistaken for a miss in five hundred. A miss still exits 3,
never 0 -- a promote that promoted nothing must be a red job -- and the detail
now names the SHA, the count searched, the bound, and whether the search reached
the end of the history (the build never uploaded this commit) or stopped at the
bound (raise `--max-versions`, or use `--version-id`).
