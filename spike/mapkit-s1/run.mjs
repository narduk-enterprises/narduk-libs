...
const scenarios = {
  ...
  // Q-C without a 30-minute wait: authorizationCallback path whose FIRST token is already
  // expired. If MapKit re-invokes the callback to retry, the refresh wiring is proven now.
  refreshRetry: { host: HOST_A, port: PORT_A, csp: null, tokenOrigin: ORIGIN_A, iatOffset: -7200, query: 'authCallback=1' },
  // Second token is valid: proves the callback path recovers, not just retries.
  refreshRecover: { host: HOST_A, port: PORT_A, csp: null, tokenOrigin: ORIGIN_A, firstExpired: true, query: 'authCallback=1' },
...
const { server, state, origin } = await startServer({
  ...
  tokenPlan: (n) => ({
    origin: cfg.tokenOrigin,
    ttlSeconds: ttlFor,
    iatOffset: cfg.firstExpired ? (n === 1 ? -7200 : 0) : (cfg.iatOffset || 0),
  }),
});
