 ...
 * absent.
 */
export function buildWalkthrough(catalog, options) {
    const runs = [];
    const missing = [];
    ...
    if (revisions.length > 1 && !options.allowMixedAppRevision) {
        throw new Error(`refusing to assemble a walkthrough across application revisions [${revisions.join(', ')}] ` +
    ...
    const sections = runs.map(({ manifest }) => {
        ...
        const steps = manifest.steps
            ...
            const shot = step.shot
                ...
                : '';
            const skipped = step.status === 'skipped-not-applicable'
        ...
        return [
            ...
            `<p class="meta">as ${escapeHtml(journey?.role ?? '')} · world ${escapeHtml(manifest.scenario.id)} · app ${escapeHtml(manifest.appRevision)} · run ${escapeHtml(manifest.startedAt)}</p>`,
            `<ol>${steps}</ol>`,
    ...
    const html = [
        ...
        `<p class="meta">declaration ${escapeHtml(options.currentDigest.slice(0, 18))} · profile ${escapeHtml(options.profileName)}</p>`,
        ...sections,
    ].join('\n');
