...
export function defineCatalog(catalog) {
    ...
    for (const scenario of catalog.scenarios) {
        if (!KEBAB.test(scenario.id))
            note(`scenario "${scenario.id}": id must be kebab-case`);
        if (scenarioIds.has(scenario.id))
            note(`scenario "${scenario.id}": duplicate id`);
        scenarioIds.add(scenario.id);
        if (!scenario.name.trim())
            note(`scenario "${scenario.id}": name is empty`);
        if (!scenario.blurb.trim())
            note(`scenario "${scenario.id}": blurb is empty`);
    }
    if (catalog.scenarios.length === 0)
        note('catalog declares no scenarios');
    if (Object.keys(catalog.profiles).length === 0)
        note('catalog declares no profiles');
    for (const [name, profile] of Object.entries(catalog.profiles)) {
        if (!KEBAB.test(name))
            note(`profile "${name}": name must be kebab-case`);
        if (profile.kind === 'web') {
    ...
    for (const journey of catalog.journeys) {
        ...
        if (!KEBAB.test(journey.id))
            note(`${where}: id must be kebab-case`);
        if (journeyIds.has(journey.id))
            note(`${where}: duplicate id`);
        journeyIds.add(journey.id);
        if (!journey.title.trim())
            note(`${where}: title is empty`);
        if (!journey.outcome.trim())
            note(`${where}: outcome is empty`);
        if (journey.scenarios.length === 0)
            note(`${where}: declares no scenarios`);
        for (const scenarioId of journey.scenarios) {
            if (!scenarioIds.has(scenarioId))
                note(`${where}: unknown scenario "${scenarioId}"`);
        }
        ...
        if (journey.steps.length === 0)
            note(`${where}: declares no steps`);
        const stepIds = new Set();
        for (const step of journey.steps) {
            const stepWhere = `${where} step "${step.id}"`;
            if (!KEBAB.test(step.id))
                note(`${stepWhere}: id must be kebab-case`);
            if (stepIds.has(step.id))
                note(`${stepWhere}: duplicate id`);
            stepIds.add(step.id);
            if (!step.say.trim())
                note(`${stepWhere}: say is empty`);
            if (step.skipWhen) {
                ...
                for (const scenarioId of step.skipWhen.scenarios) {
                    if (!journey.scenarios.includes(scenarioId)) {
                        note(`${stepWhere}: skipWhen names "${scenarioId}", not a scenario this journey declares`);
                    }
                }
        ...
        if (journey.surface !== 'web') {
            ...
            for (const field of ['xcTarget', 'xcClass', 'xcMethod']) {
                if (!binding[field]?.trim())
                    note(`${where}: binding.${field} is empty`);
            }
        }
    }
    for (const story of catalog.stories ?? []) {
        for (const journeyId of story.journeys) {
            if (!journeyIds.has(journeyId))
                note(`story "${story.id}": unknown journey "${journeyId}"`);
        }
    }
    for (const sequence of catalog.sequences ?? []) {
        ...
        for (const journeyId of sequence.journeys) {
            ...
            if (journey.surface !== sequence.surface) {
                note(`${where}: journey "${journeyId}" is ${journey.surface}, sequence is ${sequence.surface}`);
            }
...
export const defineScenario = (scenario) => scenario;
export const defineJourney = (journey) => journey;
export const defineAudience = (audience) => audience;
export const defineStory = (story) => story;
export const defineSequence = (sequence) => sequence;
export const defineProfiles = (profiles) => profiles;
