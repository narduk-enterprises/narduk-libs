 ...
 */
export function expectedStepIds(journey, scenarioId) {
 ...
 */
export function verifyRun(catalog, manifest, runDirectory, options = {}) {
    const issues = [];
    ...
    if (!journey)
        return [`manifest names unknown journey "${manifest.journey}"`];
    ...
    if (manifest.scenario.generationAfter !== manifest.scenario.generation) {
        issues.push(`world generation changed mid-run ("${manifest.scenario.generation}" to ` +
    ...
    if (executedIds.join(' ') !== expected.join(' ')) {
        issues.push(`manifest step sequence [${executedIds.join(', ')}] does not equal declared ` +
    ...
    for (const [index, step] of manifest.steps.entries()) {
        ...
        if (step.status === 'skipped-not-applicable') {
            const declaresProbe = journey.surface === 'web' && 'appliesIf' in declared && declared.appliesIf;
            if (!declaresProbe) {
    ...
    if (options.currentDigest !== undefined && options.currentDigest !== manifest.declarationDigest) {
        issues.push('declaration digest mismatch: this run executed a catalog that has since changed ' +
 ...
 */
export function promoteRun(catalog, manifest, runDirectory, options) {
...
export function readRunManifest(runDirectory) {
...
export function runPaths(options) {
    const modeDirectory = join(options.outRoot, options.environment, options.surface, options.journeyId, options.profileName, options.mode);
    return {
        modeDirectory,
