...
describe('create-narduk-app generation contract', () => {
  ...
  it('emits every generator-parity template added for Buoys-shape parity', () => {
    for (const { capabilities, hasDatabase, hasAuth, label } of [
      ...
      // narduk-libs#787: the promotion can also be dispatched by ci.yml, so
      // the gate promotes only main's head and, for a dispatch, re-reads the
      // latest `ci / Required` on that exact SHA before anything deploys.
      expect(runbook, label).toContain('  workflow_dispatch:\n    inputs:\n      verified-sha:')
      expect(runbook, label).toContain('if [ "$head" != "$CANDIDATE" ]; then')
...
describe('generated app typecheck and lint surfaces', () => {
  ...
  it('emits a .github/workflows/dependabot-merge.yml that triggers on CI completion and merges only the safe lane', () => {
    for (const { capabilities, label } of capabilitySets) {
      ...
      expect(ciWorkflow, label).toContain("grep -Eq '^ {6}verified-sha:[[:space:]]*$'")
