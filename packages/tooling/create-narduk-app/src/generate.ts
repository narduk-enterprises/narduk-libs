  ...
  const files: GeneratedFile[] = [
    ...
    },
    {
      ...
      contents: text(
        ...
        '',
        "The promote job runs on `workflow_run` after a push CI run goes green, and also on `workflow_dispatch` with a verified SHA. Dependabot merges start main CI with this repository's `GITHUB_TOKEN`; that dispatched run emits no `workflow_run` (narduk-libs#787), so `.github/workflows/dependabot-merge.yml` waits for CI and then starts Promote with the SHA that run verified. Resolve the version Workers Builds uploaded for **the commit that run verified**, deploy it at 100%, and then prove it:",
        '',
        '```yaml',
        ...
        '    inputs:',
        '      verified-sha:',
        '        description: CI-verified SHA (Dependabot merge path; narduk-libs#787)',
        '        required: true',
        '        type: string',
        'env:',
        "  VERIFIED_SHA: ${{ github.event_name == 'workflow_dispatch' && inputs.verified-sha || github.event.workflow_run.head_sha }}",
        'steps:',
        '  - id: promote',
