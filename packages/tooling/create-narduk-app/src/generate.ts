  ...
  const files: GeneratedFile[] = [
    ...
    },
    {
      ...
      contents: text(
        ...
        '',
        'Worker secrets are injected at runtime only and are not visible to `nuxt build`. Apps that enable runtime OG image generation (the `seo` capability default) must set `NUXT_OG_IMAGE_SECRET` as a Workers Builds _Build variable_ or the build throws. Set `NUXT_SESSION_PASSWORD` the same way. CI uses committed test-only placeholders; production must use the real Vault-issued values, never those placeholders. `build:ci` exits before it exports them when `WORKERS_CI`, `WORKERS_CI_BRANCH`, or `NARDUK_ALLOW_LOCAL_WRANGLER_DEPLOY` is set. A successful `build:ci` also writes `apps/web/.output/.narduk-build-ci`, and `cf:deploy`, `cf:deploy:preview`, `deploy`, and `deploy:dry-run` refuse to upload an output that still has that marker, so `quality:static` followed by `pnpm run cf:deploy` cannot publish the placeholders. A later real `pnpm run build` or `cf:build` replaces `.output` and clears the marker.',
        '',
    ...
    },
    {
      ...
      contents: text(
        ...
        "  readFile('wrangler.jsonc', 'utf8').then(parseJsonc),",
        '  Promise.resolve(JSON.parse(manifestText)),',
        '])',
        '',
        'const actual = {',
        '  cron: [...(wrangler.triggers?.crons ?? [])].sort(),',
        ...
        '',
        // wrangler.jsonc is JSONC: `//` and `/* */` comments, and trailing
        // commas (this file's own "preview_urls": ...,\n} has one). Both are
        // stripped by walking the text, never by regex: a regex cannot tell
        // syntax from string content, and this file always holds the string
        // "**/*.mjs", whose `/*` a regex read as a comment running to the
        // next `*/` -- the one in a "*/15 * * * *" cron (narduk-libs#914).
        'function parseJsonc(text) {',
        '  try {',
        '    return JSON.parse(dropTrailingCommas(dropComments(text)))',
        '  } catch (error) {',
        '    throw new Error(`wrangler.jsonc is not valid JSONC: ${error.message}`)',
        '  }',
        '}',
        '',
        '/** Offset just past the string that opens at `start`. */',
