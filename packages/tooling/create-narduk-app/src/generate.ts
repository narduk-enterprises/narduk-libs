  ...
  const files: GeneratedFile[] = [
    ...
    },
    {
      ...
      contents: text(
        ...
        '    steps:',
        "      # Promote only main's head: an older commit whose CI finished late must",
        '      # not replace a newer one. A dispatch is only a claim, so it also',
        '      # re-checks the latest ci / Required on that exact commit.',
        '      - id: gate',
        '        env:',
        '          GH_TOKEN: ${{ github.token }}',
        '          REPO: ${{ github.repository }}',
        '          EVENT: ${{ github.event_name }}',
        "          CANDIDATE: ${{ github.event_name == 'workflow_dispatch' && inputs.verified-sha || github.event.workflow_run.head_sha }}",
        '        run: |',
        '          set -euo pipefail',
        '          [[ "$CANDIDATE" =~ ^[0-9a-f]{40}$ ]]',
        '          head=$(gh api "repos/$REPO/commits/main" --jq .sha)',
        '          if [ "$head" != "$CANDIDATE" ]; then',
        '            echo "::notice::main is at $head, not $CANDIDATE; the newer commit is promoted on its own."',
        '            exit 0',
        '          fi',
        '          if [ "$EVENT" = workflow_dispatch ]; then',
        '            passed=$(gh api "repos/$REPO/commits/$CANDIDATE/check-runs?check_name=ci%20%2F%20Required" \\',
