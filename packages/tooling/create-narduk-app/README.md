...
```sh
pnpm dlx @narduk-enterprises/create-narduk-app@0.13.4 harbor-notes \
...
| `AGENTS.md`                                 | app, with one generator-owned region    | Only the text between `<!-- narduk:router:start -->` and `<!-- narduk:router:end -->`. The heading, the app's prose and every app-added section are never read. An existing file that predates the markers receives the block on `upgrade --write`. A missing file is left absent.                                                                                                                                                                                                                                     |
...
An existing `AGENTS.md` with no `narduk:router` markers gets that block appended
on `upgrade --write`; the rest of the file is untouched (narduk-libs#377).
`docs/e2e-testing.md` still reports `unmanaged` until someone adds the
`narduk:e2e-policy` markers. The sanctioned opt-out for either file is a
`narduk:unmanaged` header. Apps generated from this version already carry the
markers. Upgrade does not create a missing `AGENTS.md`.
