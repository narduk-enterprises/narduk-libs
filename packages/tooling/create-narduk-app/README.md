...
- **Layout.** `apps/web/nuxt.config.ts` or `apps/web/package.json` is the
  ...
  app, is a root Nuxt app. Its migrate scripts are
  `wrangler d1 migrations apply <database> --local` and `--remote`, using the
  database name in the wrangler file. They are not `pnpm --filter web`.
- **Migrate scripts.** On an `apps/web` app, `db:migrate:local` and
  ...
  that script. A working root command is left alone.
- **Database.** `databaseBackend: 'none'` or `'d1'` in the Nuxt config wins.
  ...
  the wrangler file (`d1_databases`) selects `d1`. No binding selects `none`,
  and the migrate scripts are not proposed. `--database` still overrides.
- **Port.** A literal `devServer.port` in that Nuxt config is the dev port.
  Otherwise `narduk.localDevNuxtPort`, otherwise 3000. A non-literal expression
  such as `port: resolvedLocalNuxtPort` is not guessed.
- **Wrangler.** The managed file is `Config/project-lifecycle.json`'s
  `nativeManifests.wrangler` when that path exists, otherwise the first
  `wrangler.jsonc` or `wrangler.json` under `apps/web` or at the repository
  root. `upgrade` still only writes top-level `cache.enabled` and does not
  create the file.
- **Session password.** `build:ci` includes the test-only
  `NUXT_SESSION_PASSWORD` only when the app's capabilities include `auth`.
  Setting it on any other app turns `nuxt-auth-utils` on. The OG-image
  placeholder stays, because `seo` builds fail closed without it.
...
| wrangler config (`wrangler.json` or `wrangler.jsonc`, at `apps/web/` or the root, or `nativeManifests.wrangler`) | app, with one generator-owned JSONC key | Only the top-level `"cache": { "enabled": true }` object (narduk-libs#672). The path is the file the checkout already has. Bindings, routes, `account_id`, env blocks and comments stay app-owned. An explicit `"enabled": false` is left alone, and a `cache` with other keys but no flag is reported unresolved rather than rewritten. Missing `cache` is the pre-#658 gap: existing apps never received Workers Cache because this file was seeded. `upgrade` does not create the file.                             |
