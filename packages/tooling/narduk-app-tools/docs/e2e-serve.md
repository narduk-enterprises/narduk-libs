...
- Depend on a `wrangler` install inside this package. Resolve wrangler from
  the app; if it is missing the process prints exactly
...
and `[e2e-serve] still starting the worker...` every 15s until ready. Do not
set Playwright `webServer.stderr: 'ignore'` — that would drop the heartbeat
too.
...
That block (including esbuild blank-line wrapping and Playwright's
`[WebServer]` prefix) is swallowed. Every other `✘ [ERROR]`, and a
`Broken pipe` that is not the `::write` disconnect form, still prints. A
...
`E2E_PREBUILT_ARTIFACT=1` and `nuxt dev` otherwise. Existing apps replace
their `apps/web/scripts/serve-e2e-build.mjs` copy after this package is
published.
