...
| 9.6       | A Nitro file that mounts `/api/health` (`server/api/health.*` or `server/api/health/index.*`, including under `apps/web`) and never references `registerHealthCheck`. `server/api/v1/health.get.ts` mounts `/api/v1/health` and is not this check | `fail` if narduk-core is a dependency, else `unknown` (WARN)                       |
