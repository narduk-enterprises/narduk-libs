...
import type { RegistryPublication } from '../../src/foundation/npm-registry.js'
...
describe('item 8 -- shared-ui-pinned', () => {
  ...
  it('passes a UI app that pins every published shared-UI package exactly (shell unpublished today)', async () => {
    const root = makeTempRepo()
    tempDirs.push(root)
    writeUiApp(root, { [UI]: '0.1.2', [CHARTS]: '1.2.0' })
    const checks = await evaluate(root)
    expect(statusOf('8.0', checks)).toBe('pass')
    expect(statusOf('8.1', checks)).toBe('not-applicable')
    expect(detailOf('8.1', checks)).toContain('nothing is published')
    expect(statusOf('8.2', checks)).toBe('pass')
    expect(statusOf('8.3', checks)).toBe('pass')
  })

  it('fails a UI app missing published narduk-ui / narduk-charts (intended #253 enforcement)', async () => {
    const root = makeTempRepo()
    tempDirs.push(root)
    writeUiApp(root)
    const checks = await evaluate(root)
    expect(statusOf('8.1', checks)).toBe('not-applicable')
    expect(statusOf('8.2', checks)).toBe('fail')
    expect(detailOf('8.2', checks)).toContain(`${UI} is published (latest 0.1.2)`)
    expect(detailOf('8.2', checks)).toContain('does not depend on it')
    expect(detailOf('8.2', checks)).toContain('add an exact pin')
    expect(statusOf('8.3', checks)).toBe('fail')
    expect(detailOf('8.3', checks)).toContain(`${CHARTS} is published (latest 1.2.0)`)
  })

  it.each([
    { id: '8.1' as const, pkg: SHELL, latest: '1.0.0' },
    { id: '8.2' as const, pkg: UI, latest: '2.3.4' },
    { id: '8.3' as const, pkg: CHARTS, latest: '1.2.0' },
  ])(
    '$id covers unpublished / unreadable / published-absent / exact-pin for $pkg',
    async ({ id, pkg, latest }) => {
      const root = makeTempRepo()
      tempDirs.push(root)
      writeUiApp(root)

      expect(statusOf(id, await evaluate(root, { [pkg]: { status: 'unpublished' } }))).toBe(
        'not-applicable',
      )
      expect(detailOf(id, await evaluate(root, { [pkg]: { status: 'unpublished' } }))).toContain(
        'nothing is published',
      )

      expect(statusOf(id, await evaluate(root, { [pkg]: { status: 'unreadable' } }))).toBe(
        'unknown',
      )

      const absent = await evaluate(root, {
        [pkg]: { status: 'published', latest, major: Number(latest.split('.')[0]) },
      })
      expect(statusOf(id, absent)).toBe('fail')
      expect(detailOf(id, absent)).toContain(`${pkg} is published (latest ${latest})`)
      expect(detailOf(id, absent)).toContain('does not depend on it — add an exact pin')

      writeUiApp(root, { [pkg]: latest })
      const present = await evaluate(root, {
        [pkg]: { status: 'published', latest, major: Number(latest.split('.')[0]) },
      })
      expect(statusOf(id, present)).toBe('pass')
    },
  )
...
describe('item 8 -- shared-ui-pinned artefact runner', () => {
  ...
  it('rolls a UI app with exact published pins and unpublished shell up to PASS / exit 0', async () => {
    const root = makeTempRepo()
    tempDirs.push(root)
    writeUiApp(root, { [UI]: '0.1.2', [CHARTS]: '1.2.0' })
  ...
  it('rolls a UI app missing a published package up to FAIL / exit 1', async () => {
    const root = makeTempRepo()
    tempDirs.push(root)
    writeUiApp(root)
    const artefact = await run(root)
  ...
  it('rolls an unreadable registry up to UNKNOWN / exit 2', async () => {
    const root = makeTempRepo()
    tempDirs.push(root)
    writeUiApp(root, { [UI]: '0.1.2', [CHARTS]: '1.2.0' })
  ...
  it('rolls a missing package.json up to UNKNOWN / exit 2', async () => {
    const root = makeTempRepo()
    tempDirs.push(root)
    const artefact = await run(root)
    expect(artefact.item.status).toBe('unknown')
  ...
  it('requires narduk-shell once it is published', async () => {
    const root = makeTempRepo()
    tempDirs.push(root)
    writeUiApp(root, { [UI]: '2.3.4', [CHARTS]: '1.2.0' })
    const artefact = await run(root, ALL_PUBLISHED)
    expect(artefact.item.status).toBe('fail')
    expect(artefact.item.checks.find((c) => c.id === '8.1')?.detail).toContain(
      `${SHELL} is published (latest 1.0.0)`,
    )
  })
})
