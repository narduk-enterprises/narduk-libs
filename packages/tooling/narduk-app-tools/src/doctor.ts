 ...
 * re-bundles it with esbuild and breaks the dynamic `import()` Nuxt uses for its
 * server error component, so every 404 and 500 renders an empty shell. That
 * reached production in six apps before anything checked (narduk-libs#245). It
 * is a warning, not a failure, so a patch release does not turn an app's doctor
 * red. The detail names the consequence.
 */
