...
links to the same generated `tokens.css`; `_ds_manifest.json` has exactly one
`globalCssPaths` entry. `build-manifest.json` records package versions, input
hashes, output hashes and coverage. Rebuilding the same source and lockfile
produces identical bytes. Source data uses fixed demonstration values. No
production services or remote fonts are needed.

The coded `narduk-ui/tokens.css` and compiled component styles are
authoritative. This tool never reads canvas exports, emits React, publishes a
package, or contacts Claude Design. The fixtures are authored Vue examples; the
HTML and token bundle are generated.
...
`design-system-build` skill in agent-infrastructure drives account selection and
upload review separately.
