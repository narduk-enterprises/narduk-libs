...
function upgradeAuthorizerName(index) {
 ...
 */
function upgradeEntries(upgrades) {
 ...
 */
export function buildWorkerEntrySource(nitroEntry, durableObjects, upgrades = []) {
    const classExports = durableObjects.map(({ className, modulePath }) => `export { ${className} } from ${JSON.stringify(modulePath)}`);
    ...
    return [
        ...
        ...upgrades.flatMap((upgrade, index) => upgrade.authorizeModulePath === undefined
            ? []
            : [
                `import ${upgradeAuthorizerName(index)} from ${JSON.stringify(upgrade.authorizeModulePath)}`,
            ]),
        ...classExports,
        '',
 ...
 */
export function createWorkerEntryHook(durableObjects, upgrades = []) {
    return (nitro, rollupConfig) => {
        ...
        writeFileSync(entryPath, buildWorkerEntrySource(nitro.options.entry, durableObjects, upgrades), 'utf8');
