...
function resolveModulePath(className, modulePath, rootDir) {
    ...
    if (!found) {
        throw new NardukRealtimeConfigurationError(`realtime.durableObjects.${className} points at "${modulePath}", which does not resolve to a file. Tried: ${MODULE_SUFFIXES.map((suffix) => base + suffix).join(', ')}.`);
    }
    return found;
}
/**
