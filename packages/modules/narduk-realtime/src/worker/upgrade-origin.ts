 ...
 */
export function parseUpgradeOrigin(value: string): UpgradeOriginParse {
  ...
  if (candidate.includes('*')) {
    return {
      ok: false,
      reason:
        "is a wildcard, which would let any site open a socket with the viewer's cookies attached: name every origin instead",
    }
  }
...
export function sameOriginFor(host: string): string {
