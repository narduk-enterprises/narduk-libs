...
| `leader`                                                                                    | `{ anchor: HTMLElement \| null } \| null`                                          | `null`                    | **NEW (narduk-libs#517).** Draws a leader from the selected pin to `anchor` on every region change and when `items` move the selection. Emits `leader-offscreen` only when the pin leaves the map frame. The overlay is also `MapKitLeaderOverlay` on `./client`.                                                                                      |
