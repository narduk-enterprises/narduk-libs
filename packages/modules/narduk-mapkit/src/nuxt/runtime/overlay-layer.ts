...
const DEFAULT_POLYGON_STYLE: OverlayStyle = {
  fillColor: '#10b981',
  fillOpacity: 0.2,
  lineWidth: 1.5,
  strokeColor: '#065f46',
...
const DEFAULT_LINE_STYLE: OverlayStyle = {
  fillColor: '#000000',
  fillOpacity: 0,
  lineWidth: 3,
  strokeColor: '#0284c7',
...
function ringsFor(
  ...
  const toRings = (polygon: unknown): MapKitCoordinateLike[][] => {
...
export class MapKitOverlayLayer {
  readonly #circles: OverlayLike[] = []
  readonly #features = new WeakMap<object, GeoJSONFeature>()
  readonly #options: MapKitOverlayLayerOptions
  readonly #overlays: OverlayLike[] = []
  ...
  setGeoJSON(collection: GeoJSONFeatureCollection | null): void {
    ...
    for (const feature of collection.features) {
      if (feature.geometry.type === 'LineString') {
        const points = mapKitGeometryPoints(feature.geometry).map(
      ...
      for (const rings of polygons) {
        ...
        this.#add(
          new mapkit.PolygonOverlay(coordinates, { style: this.#style(style, 0.2) }),
          feature,
        )
      }
    }
  ...
  setCircles(circles: readonly MapKitCircle[]): void {
    ...
    for (const circle of circles) {
      ...
      const overlay = new mapkit.CircleOverlay(
        new mapkit.Coordinate(circle.lat, circle.lng),
        circle.radius,
        { style },
      )
  ...
  resizeCirclesToRegion(latitudeDelta: number): void {
    ...
    if (!options.dynamicCircleRadius || this.#circles.length === 0) return
    const raw = latitudeDelta * METRES_PER_DEGREE_LATITUDE * (options.circleScaleFactor ?? 0.004)
    const radius = Math.max(
      options.minCircleRadius ?? 200,
      Math.min(options.maxCircleRadius ?? 6000, raw),
    )
    for (const circle of this.#circles) circle.radius = radius
  }
  ...
  #style(style: OverlayStyle, defaultFillOpacity: number): object {
  ...
  #add(overlay: OverlayLike, feature: GeoJSONFeature): void {
