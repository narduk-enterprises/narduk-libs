...
export function createFakeMapKit(options: FakeMapKitOptions = {}): FakeMapKitHandle {
  const runtime: FakeMapKitRuntime = createFakeMapKitRuntime(options)
  return {
    inspect: runtime.inspect,
    install(target: Record<string, unknown> = globalThis as unknown as Record<string, unknown>) {
      const hadOwn = Object.hasOwn(target, 'mapkit')
      const previous = target['mapkit']
      target['mapkit'] = runtime.mapkit
      return () => {
        if (hadOwn) target['mapkit'] = previous
        else delete target['mapkit']
      }
    },
    load: runtime.load,
 ...
 * different realm.
 */
export function isFakeMapKitNotImplemented(value: unknown): value is FakeMapKitNotImplementedError {
  return (
