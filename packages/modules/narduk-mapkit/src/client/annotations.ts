...
interface MapKitAnnotationRegistryEntry<TAnnotation> {
  annotation: TAnnotation
  signature: string
}

function requireAnnotationKey(key: string): void {
  if (!key.trim()) throw new Error('annotation key is required')
}

export class MapKitAnnotationRegistry<TAnnotation> {
  readonly #entries = new Map<string, MapKitAnnotationRegistryEntry<TAnnotation>>()
  readonly #map: MapKitAnnotationMapHandle<TAnnotation>
  #destroyed = false
  ...
  register(descriptor: MapKitAnnotationDescriptor<TAnnotation>): TAnnotation {
    if (this.#destroyed) throw new Error('annotation registry is destroyed')
  ...
  reconcile(
    ...
    const removals: TAnnotation[] = []
    const additions: TAnnotation[] = []
    ...
    if (removals.length > 0) this.#map.removeAnnotations(removals)
    if (additions.length > 0) this.#map.addAnnotations(additions)
