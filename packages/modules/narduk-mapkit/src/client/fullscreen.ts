 ...
 */
function overlayCss(mode: MapKitFullscreenMode, zIndex: number): string {
  const fill = 'width: 100%; height: 100%; max-width: none; max-height: none; margin: 0;'
  if (mode === 'fullscreen') return fill
 ...
 */
function appendCss(base: string, addition: string): string {
  const trimmed = base.trim()
  if (trimmed === '') return addition
  return trimmed.endsWith(';') ? `${trimmed} ${addition}` : `${trimmed}; ${addition}`
}

function resolveDocument(options: MapKitFullscreenControllerOptions): MapKitFullscreenDocument {
...
export class MapKitFullscreenController {
  readonly #defaultMode: MapKitFullscreenMode
  readonly #document: MapKitFullscreenDocument
  readonly #element: MapKitFullscreenElement
  readonly #exitOnEscape: boolean
  readonly #listeners = new Set<MapKitFullscreenListener>()
  readonly #lockScroll: boolean
  readonly #onLayout: ((event: MapKitFullscreenChangeEvent) => void) | undefined
  readonly #zIndex: number

  #destroyed = false
  #escapeAttached = false
  #mode: MapKitFullscreenMode | null = null
  /** Identity of the in-flight native request, so a late rejection is ignored. */
  #nativeRequest: object | null = null
  #savedBodyOverflow: string | null = null
  #savedCssText: string | null = null
  #savedRootOverflow: string | null = null
  ...
  readonly #onFullscreenError = (): void => {
    ...
    if (this.#destroyed || this.#nativeRequest === null) return
    this.#nativeRequest = null
    this.#fallback('rejected')
  }

  readonly #onKeyDown = (event: MapKitFullscreenKeyboardEventLike): void => {
    if (this.#mode !== 'viewport') return
   ...
   */
  async enter(mode: MapKitFullscreenMode = this.#defaultMode): Promise<MapKitFullscreenState> {
    if (this.#destroyed || this.#mode === mode) return this.#state()
    if (mode === 'fullscreen') return this.#enterNative()
   ...
   */
  async exit(): Promise<MapKitFullscreenState> {
    if (this.#mode === null) return this.#state()
    this.#teardown()
    return this.#settle('exit', null)
  }

  /** Enter when inactive, exit when active. `mode` is ignored while active. */
  async toggle(mode: MapKitFullscreenMode = this.#defaultMode): Promise<MapKitFullscreenState> {
  ...
  destroy(): void {
    ...
    if (wasActive) this.#settle('destroy', null)
    this.#listeners.clear()
  }

  async #enterNative(): Promise<MapKitFullscreenState> {
    const element = this.#element
    const token = {}
    ...
    try {
      ...
      if (this.#nativeRequest !== token) return this.#state()
      this.#nativeRequest = null
    ...
    if (this.#destroyed || this.#nativeRequest !== token) return this.#state()
    this.#nativeRequest = null

    // A document that fires `fullscreenchange` has already adopted the session;
    // one that only resolves the promise is adopted here.
    if (this.#mode === 'fullscreen') return this.#state()
    this.#teardown()
    this.#present('fullscreen')
    return this.#settle('enter', 'fullscreen')
  }

  #fallback(cause: MapKitFullscreenFallbackCause): MapKitFullscreenState {
    this.#teardown()
    this.#present('viewport')
  ...
  #present(mode: MapKitFullscreenMode): void {
    const element = this.#element
  ...
  #teardown(): void {
    ...
    if (this.#savedCssText !== null) this.#element.style.cssText = this.#savedCssText
    this.#savedCssText = null
    this.#element.removeAttribute(MAPKIT_FULLSCREEN_ATTRIBUTE)
    if (mode === 'fullscreen') this.#releaseNative()
  }
  ...
  #unlockDocumentScroll(): void {
    const root = this.#document.documentElement
    if (root && this.#savedRootOverflow !== null) root.style.overflow = this.#savedRootOverflow
    this.#savedRootOverflow = null

    const body = this.#document.body
    if (body && this.#savedBodyOverflow !== null) body.style.overflow = this.#savedBodyOverflow
  ...
  #attachEscape(): void {
    if (!this.#exitOnEscape || this.#escapeAttached) return
  ...
  #state(): MapKitFullscreenState {
    return { active: this.#mode !== null, mode: this.#mode }
  }

  #settle(
    reason: MapKitFullscreenReason,
    requestedMode: MapKitFullscreenMode | null,
    fallbackCause: MapKitFullscreenFallbackCause | null = null,
  ): MapKitFullscreenState {
    const event: MapKitFullscreenChangeEvent = {
...
export function createMapKitFullscreenController(
  options: MapKitFullscreenControllerOptions,
