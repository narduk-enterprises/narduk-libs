...
import type { MkAnnotation, MkMap, MkRuntime } from './runtime.js'
 ...
 */
function repaint(annotation: MkAnnotation, next: HTMLElement): void {
  const { element } = annotation
  for (const name of element.getAttributeNames()) {
    if (!HOST_ATTRIBUTES.has(name) && !next.hasAttribute(name)) element.removeAttribute(name)
  }
...
function descriptorSignature(spec: MarkSpec): string {
 ...
 */
export function createMarkLayer(map: MkMap, runtime: MkRuntime): MarkLayer {
  const registry = new MapKitAnnotationRegistry<MkAnnotation>({ map })
  ...
  function assignKey(spec: MarkSpec, freed: string[]): string {
    if (spec.selected) return spec.key
  ...
  return {
    ...
    render: (specs) => {
      ...
      for (const [specKey, slotKey] of slotOf) {
        if (!present.has(specKey)) freed.push(slotKey)
      }
      ...
      registry.reconcile(
        specs.map((spec) => {
          ...
          if (!spec.selected) nextSlotOf.set(spec.key, key)
          return {
            create: () =>
              new runtime.Annotation(new runtime.Coordinate(spec.lat, spec.lon), spec.build, {
            ...
            update: (annotation: MkAnnotation) => {
      ...
      slotOf.clear()
      for (const [specKey, slotKey] of nextSlotOf) slotOf.set(specKey, slotKey)
    },
  }
}
