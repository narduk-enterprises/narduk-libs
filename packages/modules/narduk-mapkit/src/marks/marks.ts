...
export type PinGlyph =
  | { bearing: number; kind: 'notch' }
  | { bearing: number; extent: number; fill: string; kind: 'path'; stroke: string }
...
function paintDisc(disc: HTMLElement, paint: PinPaint): void {
  ...
  if (paint.glyph !== null) drawGlyph(disc, paint.glyph, paint)
...
function notchPoints(r: number): { left: string; right: string; tip: string } {
...
function glyphParts(glyph: PinGlyph, paint: PinPaint): SVGElement[] {
  if (glyph.kind === 'path') {
    return [
...
function drawGlyph(disc: HTMLElement, glyph: PinGlyph, paint: PinPaint): void {
  ...
  for (const part of glyphParts(glyph, paint)) svg.appendChild(part)
  disc.appendChild(svg)
}

function stackRim(stack: PinStack, radius: number): HTMLElement {
...
export function createPinMark(
  ...
  if (target.stack) button.appendChild(stackRim(target.stack, target.paint.radius))
  const disc = el('span', 'mk-pin-disc')
  paintDisc(disc, target.paint)
  button.appendChild(disc)
  root.appendChild(button)
  if (target.name) root.appendChild(pinName(target.name, target.paint.radius))
  return root
}

/** A name centred below its disc; decorative, as the button's label already names it. */
function pinName(name: string, radius: number): HTMLElement {
...
export function createSelectedMark(
  target: MarkTarget & { flip: boolean; hitSize: number; name: string; paint: PinPaint },
): HTMLElement {
