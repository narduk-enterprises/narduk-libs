 ...
 */
function warnRetiredKeys(runtimeConfig) {
    const retired = ['mapkitAllowedOrigins'];
    for (const key of retired) {
        if (runtimeConfig[key]) {
            console.warn(`[narduk-mapkit] runtimeConfig.${key} is accepted and ignored in 2.1.0: the token's ` +
                'origin claim is built from the routed request host alone.');
        }
    }
    if (runtimeConfig.public['mapkitToken']) {
        console.warn('[narduk-mapkit] runtimeConfig.public.mapkitToken is accepted and ignored in 2.1.0: a ' +
