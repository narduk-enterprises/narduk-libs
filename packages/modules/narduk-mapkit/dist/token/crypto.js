 ...
 */
export async function importPrivateKeyForAppleMaps(pemKey) {
    if (pemKey.includes('-----BEGIN EC PRIVATE KEY-----') ||
        pemKey.includes('-----END EC PRIVATE KEY-----')) {
        throw new Error('Apple Maps private key must use PKCS#8 PEM format (BEGIN PRIVATE KEY); SEC1 EC PRIVATE KEY PEMs are not supported by Web Crypto importKey("pkcs8").');
    }
 ...
 */
export function derEcdsaSignatureToRaw(der) {
    if (der.length === 64)
        return der;
    if (der[0] !== 0x30)
        return der;
