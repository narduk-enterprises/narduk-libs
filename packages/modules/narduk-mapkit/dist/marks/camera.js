...
const TILE_SIZE = 256;
export function tierForSpan(longitudeDelta, widthPx) {
...
export function projectToFrame(rect, frame, point) {
...
export function frameToCoordinate(rect, frame, point) {
    return fromWorld({
 ...
 */
export function unpadRect(rect, frame, padding) {
...
export function padRect(rect, frame, padding) {
    const innerWidth = frame.width - padding.left - padding.right;
    const innerHeight = frame.height - padding.top - padding.bottom;
    if (innerWidth <= 0 || innerHeight <= 0)
        return rect;
...
export function frameContains(frame, point, margin = 0) {
 ...
 */
function freeSpan(extent, near, far) {
...
function rectAround(center, anchor, frame, scale) {
    return {
 ...
 */
export function rectForBox(box, frame, insets, options = {}) {
    const margin = options.margin ?? 32;
    const northWest = toWorld(box.north, box.west);
    const southEast = toWorld(box.south, box.east);
    const width = southEast.x >= northWest.x ? southEast.x - northWest.x : southEast.x + 1 - northWest.x;
    const height = southEast.y - northWest.y;
    const free = freeBox(frame, insets);
    const minScale = 1 / (TILE_SIZE * 2 ** (options.maxZoom ?? 11));
    const scale = Math.max(minScale, width / Math.max(1, free.width - 2 * margin), height / Math.max(1, free.height - 2 * margin));
 ...
 */
export function rectRevealing(rect, frame, insets, point, margin = 48) {
    const free = freeBox(frame, insets);
    const inside = Math.abs(point.x - free.x) <= Math.max(0, free.width / 2 - margin) &&
...
export function zoomRect(rect, factor) {
