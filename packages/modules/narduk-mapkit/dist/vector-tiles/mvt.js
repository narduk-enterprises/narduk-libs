 ...
 */
export function createMvtDecoder(options = {}) {
...
export function decodeMvtTile(bytes, options = {}) {
    return decodeMvt(bytes, options.layers ? new Set(options.layers) : null, options.properties ? new Set(options.properties) : null);
}
function decodeMvt(bytes, layerFilter, propertyFilter) {
    if (bytes.length === 0)
        return null;
    let tile;
    try {
    ...
    for (const name of names) {
        ...
        if (layer)
            extent = Math.max(extent, layer.extent);
    }
    if (extent <= 0)
        return null;
    const features = [];
 ...
 */
function hasLayerField(bytes) {
    const found = { value: false };
    try {
        new PbfReader(bytes).readFields((tag, result) => {
            if (tag === 3)
                result.value = true;
        }, found);
    }
