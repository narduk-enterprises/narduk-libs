 ...
 */
export const defaultMapKitPinSizeStops = [
    { sizePx: 5, zoom: 5 },
    { sizePx: 8, zoom: 6 },
    { sizePx: 14, zoom: 7 },
    { sizePx: 18, zoom: 8 },
 ...
 */
export function createMapKitPinSizeCurve(stops = defaultMapKitPinSizeStops, options = {}) {
    const zooms = [];
    const sizes = [];
    for (const stop of stops) {
        ...
        if (stop.sizePx < 0)
            throw new RangeError('pin size stops need a non-negative sizePx');
    ...
    if (last < 0)
        throw new RangeError('a pin size curve needs at least one stop');
    ...
    return (zoom) => {
        ...
        if (!Number.isFinite(zoom) || zoom <= zooms[0])
            return first;
        if (zoom >= zooms[last])
            return top;
...
export const defaultMapKitPinSizeCurve = createMapKitPinSizeCurve();
/** The shipped floor: everything from z7 in, the top two at z6, the top at z5. */
export function defaultMapKitPinRankFloor(zoom) {
    if (zoom <= 5)
        return 3;
    if (zoom <= 6)
        return 2;
 ...
 */
export function latchedStepIndex(boundaries, value, current, hysteresis = 0) {
    ...
    while (index < boundaries.length && value >= boundaries[index] + deadband)
        index += 1;
    while (index > 0 && value < boundaries[index - 1] - deadband)
        index -= 1;
    return index;
}
/**
 ...
 * every zoom.
 */
export function latchedPinMode(sizePx, dotBelowPx, current, hysteresisPx = 0) {
    if (!(dotBelowPx > 0))
        return 'symbol';
    const deadband = current === null ? 0 : Math.max(0, hysteresisPx);
    if (current === 'dot')
        return sizePx >= dotBelowPx + deadband ? 'symbol' : 'dot';
 ...
 */
export function cullProbeZoom(zoom, visible, hysteresis) {
    if (visible === null)
        return zoom;
 ...
 */
export function mapKitZoomForSpan(options) {
 ...
 */
export class MapKitPinScalingController {
    #annotations;
    #cancelFrame;
    #classes = new Map();
    #configs;
    ...
    track(key, classId) {
        ...
        if (previous !== undefined)
            this.#classes.get(previous)?.keys.delete(key);
    ...
    reconcile(pins) {
        ...
        for (const pin of pins) {
            ...
            if (desired.has(pin.key))
                throw new Error(`duplicate pin key "${pin.key}" in reconcile()`);
            desired.add(pin.key);
     ...
     */
    flushDeferred() {
        if (this.#destroyed || this.#deferred.size === 0)
            return;
        const released = new Set();
        for (const key of [...this.#deferred]) {
            if (this.#shouldPaint && !this.#shouldPaint(key))
                continue;
            this.#deferred.delete(key);
            released.add(key);
        }
    ...
    destroy() {
        ...
        for (const key of this.#culled) {
            ...
            if (annotation !== undefined)
                this.#setVisible(annotation, true);
        }
        if (this.#container) {
            for (const property of this.#published.keys())
    ...
    #classFor(classId) {
        ...
        if (scoped && !CSS_IDENTIFIER.test(classId)) {
            throw new Error(`pin class "${classId}" declares its own sizeCurve, so its id must be a CSS identifier`);
        }
    ...
    #noteReason(reason) {
        if (REASON_RANK[reason] > REASON_RANK[this.#reason])
            this.#reason = reason;
    }
    #schedule() {
    ...
    #update() {
        ...
        if (this.#scratch.size === 0 && !zoomMoved)
            return;
    ...
    #settlePendingKey(key) {
        ...
        if (visible)
            this.#scratch.add(key);
        return settled;
    }
    #updateClass(state, zoom, floorWhenVisible, floorWhenHidden) {
        ...
        const floor = previousVisible === null
        ...
        if (mode === previousMode && visible === previousVisible)
            return;
        ...
        for (const key of state.keys) {
            ...
            if (this.#selected.has(key))
                continue;
            if (hasPending && this.#pendingKeys.has(key))
                continue;
    ...
    #applyVisibility(key, visible) {
        if (visible === !this.#culled.has(key))
            return true;
        if (!this.#annotations)
            return true;
        const annotation = this.#annotations.get(key);
    ...
    #publish(property, value) {
        if (this.#published.get(property) === value)
            return;
...
function requirePinKey(key) {
    if (!key.trim())
        throw new Error('pin key is required');
}
/**
 ...
 */
function defaultVisibilityWriter() {
...
export function createMapKitPinScalingController(options = {}) {
