...
Web twin of [GeoGridKit](https://github.com/narduk-enterprises/GeoGridKit): decode gridded geo payloads and render them either as a **full-screen sibling canvas overlay** or as **Web-Mercator raster tiles**, with **WebGL2** (primary) and a **CPU fallback**.
...
Both run the same sampling kernel, the same normalization and the same ramp, so a
tile and the overlay never disagree about what a value looks like.

## Relationship to GeoGridKit

Shared *concepts* (scale, value range, color stops, normalize math, temporal
frames) and, since 0.4.0, shared *tile geometry*: the Web-Mercator math is a
port of GeoGridKit's `GridTileMath`, pinned to values generated by calling
GeoGridKit itself.

- **GeoGridKit**: bake tiles → MapKit / `CGImage`
- **GeoGridWeb**: overlay canvas over any basemap, or bake tiles → `OffscreenCanvas`
...
For a host that owns its own tiling. One call produces the `(x, y, z, scale, data?)
=> Promise<OffscreenCanvas | null>` function such a host consumes:
...
const canvas = renderGridTile(dataset, style, { z: 8, x: 59, y: 108, side: 256 })
```
...
1. **Argument three is `z`, argument four is `scale`.** MapKit JS delivers
   ...
   renders *something* and nothing throws.
...
*also* sets a layer opacity squares it — leave the host at `1`, or pass
...
zoom. Nothing in this package can execute GLSL, so neither simulation settles
it — that needs a measurement in a real WebGL2 context, tracked as
...
**Dynamic display range** below. Hand the same style to both and they agree;
the stretch is not an overlay-only feature.
