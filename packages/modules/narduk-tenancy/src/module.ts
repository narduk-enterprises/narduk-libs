...
const PACKAGE_NAME = '@narduk-enterprises/narduk-tenancy'
...
export interface NardukTenancyModuleOptions {
  /**
   * Register `server/` for Nitro auto-imports. Turn it off to import the
   * tenancy service and guards explicitly from their subpath exports.
   */
  server?: boolean
}
 ...
 * identity and hands `createTenancy` its own drizzle database.
 *
 * Migrations are not applied by this module either — a consumer adds
 * `node_modules/@narduk-enterprises/narduk-tenancy/drizzle` to its
 * `migrations.sources.json` (see README).
 */
export default defineNuxtModule<NardukTenancyModuleOptions>({
  meta: {
    name: PACKAGE_NAME,
    configKey: 'nardukTenancy',
