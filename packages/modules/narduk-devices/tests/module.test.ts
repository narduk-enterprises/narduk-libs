...
describe('narduk-devices Nuxt module', () => {
  beforeEach(() => {
    vi.resetModules()
    vi.clearAllMocks()
  })
  ...
  it('registers the server surface and inlines the package for Nitro', async () => {
    ...
    expect(nuxt.options.build.transpile).toContain('@narduk-enterprises/narduk-devices')
    ...
    expect(
      (nuxt.options as unknown as { nitro: { externals: { inline: string[] } } }).nitro.externals
        .inline,
    ).toContain('@narduk-enterprises/narduk-devices')
  })
