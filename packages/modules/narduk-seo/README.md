...
failing the build, and `useSeo` falls back to the static image. The committed CI
placeholder is rejected on builds the estate deploys -- Workers Builds
(`WORKERS_CI`) and a local `wrangler deploy` behind
